...
| 9D — Workload demo | pending commit | [phase_9D_workload_demo_2026-05-08.txt](phase_9D_workload_demo_2026-05-08.txt) | 2026-05-08 | `run_phase9d_demo.ps1 -ComPort COM5` (90 s); scripted UART `a`/`s`/`r` + operator button presses | PASS — all 12 default patterns FOUND; ROBOTOS_APP final state=IDLE transitions=35 button=32 uart=3 ignored=0 last_byte=0x72 (operator pressed >3 times; canonical target documented as 6/3/3/0); conservation 32+3=35 ✓; OBS accepted=125 dispatched=124 pending=1 peak=16 dropped=3 (queue saturation under rapid button input observed safely); CFSR=0 HFSR=0 (19 occurrences); herr=0 unhandled=0; 37930 bytes |
