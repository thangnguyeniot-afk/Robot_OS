...
| Phase 11 through Phase 20 | [`DEVKIT_PROGRESS_PHASE_11_20.md`](../01_PROGRESS/DEVKIT_PROGRESS_PHASE_11_20.md) | Running log; Phase 12F-pre is latest closed entry (`CLOSED_DOCS_ONLY`; `PHASE_12F_PRE_RECOMMEND_HOST_BRIDGE_PROTOTYPE`); Phase 11A/11B/11C/11D/11E/11Z/12A/12B/12C/12D-pre/12D/12E-pre/12E/12F-pre closed; Phase 11A-11E sensor track complete; Phase 12A-12C Framework planning complete; Phase 12D-pre boundary explicit; Phase 12D header stub in place; Phase 12E-pre selected host unit-test consumer path; Phase 12E implementation host-test-validated (93/93 assertions); Phase 12F-pre plans Application Bridge host prototype with `FRAMEWORK_APPLICATION_BRIDGE_DRAFT.md` spec; Phase 12F implementation pending explicit user authorization. |
...
- [`FRAMEWORK_APPLICATION_BRIDGE_DRAFT.md`](../03_SPECS/FRAMEWORK_APPLICATION_BRIDGE_DRAFT.md) -- Framework Application Bridge spec (`DRAFT / NON-FINAL`; Phase 12F-pre origin; mapping engine from Adapter event keys `(type, arg0)` to Framework event IDs; caller-owned bridge instance, caller-owned FSM, caller-owned mapping table; no heap, no UART, no devkit/legacy includes; no implementation exists yet — Phase 12F pending).
