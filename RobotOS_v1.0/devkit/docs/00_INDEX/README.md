...
- [`PROBE_TRANSLATOR_ZEPHYR_BUILD_ONLY_PLAN.md`](../03_SPECS/PROBE_TRANSLATOR_ZEPHYR_BUILD_ONLY_PLAN.md) -- Probe translator Zephyr build-only admission plan (`DRAFT / NON-FINAL`; Phase 12K-pre origin; execution-ready contract for Phase 12K; locks Option A CMake strategy; defines forbidden file set (`prj.conf`/DTS/`app/CMakeLists.txt` zero-diff); 10 dependency audit rules; 10 validation gates; evidence log policy; closeout criteria; 8 risk/rollback signals; Phase 12K `NOT_STARTED`).
