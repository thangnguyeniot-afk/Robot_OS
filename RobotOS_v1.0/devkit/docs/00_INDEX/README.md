...
- [`PROBE_TRANSLATOR_FAULT_BLOCK_PLAN.md`](../03_SPECS/PROBE_TRANSLATOR_FAULT_BLOCK_PLAN.md) -- Probe translator FAULT block extension plan (`DRAFT / NON-FINAL`; Phase 12J-pre origin; execution-ready additive contract for Phase 12J; formalizes `STATE_FAULT=4u`/`EVT_FAULT=5u`/`ADAPTER_TYPE_FAULT=3u`/`ADAPTER_ARG_ANY=0xFFFFFFFFu`; ships transition rows 5-9 + mapping row 4 wildcard + state-def FAULT entry; sticky FAULT implicit via FSM no-transition; bridge `.c`/`.h` zero-diff; CMake zero-diff; TC16-TC24 additive to existing host target; 23/23 ctest target count preserved; Phase 12J `NOT_STARTED`).
